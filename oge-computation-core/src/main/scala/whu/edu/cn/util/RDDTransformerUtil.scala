...
object RDDTransformerUtil {
  ...
  import geotrellis.spark.stitch.Implicits._ // For stitch extensions
  import org.apache.spark.SparkContext
  import org.apache.spark.rdd.RDD
  import scala.reflect.ClassTag

  def saveRasterRDDToTif2(
                           input: (RDD[(SpaceTimeBandKey, MultibandTile)], TileLayerMetadata[SpaceTimeKey]),
                           outputTiffPath: String): Unit = {

    val time1 = System.currentTimeMillis()
    val (rdd, tileLayerMetadata) = input

    // === 所有操作都在驱动程序 (Driver) 层面进行 ===

    // 1. 将 RDD 的 Key 转换为 SpatialKey，为空间拼接做准备
    val spatialRDD: RDD[(SpatialKey, MultibandTile)] = rdd.map { case (key, tile) =>
      (key.spaceTimeKey.spatialKey, tile)
    }

    // 2. 转换元数据(Metadata)，使其与 SpatialKey 匹配
    //    这是从时空元数据中安全地提取空间元数据部分
    val spatialMetadata: TileLayerMetadata[SpatialKey] = {
      // 从 Bounds[SpaceTimeKey] 中提取 Bounds[SpatialKey]
      val spatialBounds: Bounds[SpatialKey] = Bounds(tileLayerMetadata.bounds.get.minKey.spatialKey,
        tileLayerMetadata.bounds.get.maxKey.spatialKey)
      tileLayerMetadata.copy(bounds = spatialBounds)
    }

    // 3. 创建一个纯空间的 TileLayerRDD
    val spatialLayer: MultibandTileLayerRDD[SpatialKey] =
      MultibandTileLayerRDD(spatialRDD, spatialMetadata)

    // 4. 调用 stitch()。这是一个行动操作，会触发一个 Spark Job 来完成分布式拼接
    println("开始分布式拼接...")
    val finalRaster: Raster[MultibandTile] = spatialLayer.stitch()
    println("拼接完成，最终栅格已返回到驱动程序。")

    // 5. 将最终拼接好的单个 Raster 写入文件
    GeoTiff(finalRaster, tileLayerMetadata.crs).write(outputTiffPath)

    val time2 = System.currentTimeMillis()
    val timeCost = time2 - time1
    println(s"成功落地tif, 耗时：$timeCost ms")
  }
