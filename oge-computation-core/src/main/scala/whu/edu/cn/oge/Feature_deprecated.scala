...
object Feature_deprecated {
  ...
  def query(productName: String = null): (String, String, ListBuffer[String]) = {
    ...
    if (conn != null) {
      try {
        ...
        sql ++= "select oge_data_product.name, oge_vector_fact.fact_data_ids, oge_vector_fact.table_name, oge_vector_fact.product_id " +
          "from oge_vector_fact join oge_data_product " +
          "on oge_vector_fact.product_id = oge_data_product.id where "
        if (productName != "" && productName != null) {
          sql ++= "name = " + "'" + productName + "'" //where name = proeuctName
        }
        println(sql)
        ...
        while (extentResults.next()) {
          ...
          productKey = extentResults.getString("product_id")
  ...
  def geometry(implicit sc: SparkContext, gjson: String, crs: String = "EPSG:4326"): RDD[(String, (Geometry, mutable.Map[String, Any]))] = {
    //异常处理，报错信息统一格式
    ...
    val geomRDD: RDD[(String,(Geometry, mutable.Map[String, Any]))] =
      if(array.getJSONObject(0).containsKey("id")){ //如果Json体中包含id属性，则使用id作为矢量的key
        ...
        for (i <- 0 until (array.size())) {
          val geom =
            if(array.getJSONObject(i).containsKey("geometry")) OGEGeometry.geometry(array.getJSONObject(i), crs).geometry
  ...
  def area(featureRDD: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:3857"): String = {
    reproject(featureRDD, crs).map(t => t._2._1.getArea).collect().toList.mkString(",")
  }
  ...
  def bounds(featureRDD: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:4326"): RDD[(String, (Geometry, Map[String, Any]))] = {
    reproject(featureRDD, crs).map(t => {
      val newGeom = t._2._1.getEnvelope
      (UUID.randomUUID().toString, (newGeom, t._2._2))
    })
  }
  ...
  def centroid(featureRDD: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:4326"): RDD[(String, (Geometry, Map[String, Any]))] = {
    reproject(featureRDD, crs).map(t => {
      val newGeom = t._2._1.getCentroid
      (UUID.randomUUID().toString, (newGeom, t._2._2))
    })
  }
  ...
  def buffer(featureRDD: RDD[(String, (Geometry, Map[String, Any]))], distance: Double, crs: String = "EPSG:3857"): RDD[(String, (Geometry, Map[String, Any]))] = {
    reproject(featureRDD, crs).map(t => {
      val newGeom = t._2._1.buffer(distance)
      newGeom.setSRID(t._2._1.getSRID)
      (UUID.randomUUID().toString, (newGeom, t._2._2))
    })
  }
  ...
  def convexHull(featureRDD: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:4326"): RDD[(String, (Geometry, Map[String, Any]))] = {
    reproject(featureRDD, crs).map(t => {
      val newGeom = t._2._1.convexHull()
      (UUID.randomUUID().toString, (newGeom, t._2._2))
    })
  }
  ...
  def reproject(featureRDD: RDD[(String, (Geometry, Map[String, Any]))], tarCrsCode: String): RDD[(String, (Geometry, Map[String, Any]))] = {
    featureRDD.map(t => {
      val srcCrsCode="EPSG:"+ t._2._1.getSRID()
      val newGeom =
        if(srcCrsCode.equals(tarCrsCode)){
          ...
          val transGeom=JTS.transform(t._2._1,transform)
          transGeom.setSRID(OGEGeometry.crsToSRID(tarCrsCode))
          transGeom
        }
      (UUID.randomUUID().toString, (newGeom, t._2._2))
    })
  ...
  def toGeoJSONString(featureRDD: RDD[(String, (Geometry, Map[String, Any]))]): String = {
    val data = featureRDD.map(t => {
      (OGEGeometry.toGeoJSONString(t._2._1), t._2._2)
    }).collect().toList
  ...
  def contains(featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val feature1 = reproject(featureRDD1, crs)
    val feature2 = reproject(featureRDD2, crs)
    feature1.first()._2._1.contains(feature2.first()._2._1).toString
  }
  ...
  def containedIn(featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val feature1 = reproject(featureRDD1, crs)
    val feature2 = reproject(featureRDD2, crs)
    feature2.first()._2._1.contains(feature1.first()._2._1).toString
  }
  ...
  def disjoint(featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val feature1 = reproject(featureRDD1, crs)
    val feature2 = reproject(featureRDD2, crs)
    feature1.first()._2._1.disjoint(feature2.first()._2._1).toString
  }
  ...
  def distance(featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val feature1 = reproject(featureRDD1, crs)
    val feature2 = reproject(featureRDD2, crs)
    feature1.first()._2._1.distance(feature2.first()._2._1).toString
  }
  ...
  def difference(implicit sc: SparkContext, featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val res = Geometry.difference(featureRDD1.first()._2._1, featureRDD2.first()._2._1, crs)
    feature(sc, res)
  }
  ...
  def intersection(implicit sc: SparkContext, featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
                   featureRDD2: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:4326"): RDD[(String, (Geometry, Map[String, Any]))] = {
    val res = Geometry.intersection(featureRDD1.first()._2._1, featureRDD2.first()._2._1, crs)
    feature(sc, res)
  }
  ...
  def intersects(featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val feature1 = reproject(featureRDD1, crs)
    val feature2 = reproject(featureRDD2, crs)
    feature1.first()._2._1.intersects(feature2.first()._2._1).toString
  }
  ...
  def symmetricDifference(implicit sc: SparkContext, featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
                          featureRDD2: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:4326"): RDD[(String, (Geometry, Map[String, Any]))] = {
    val res = Geometry.symmetricDifference(featureRDD1.first()._2._1, featureRDD2.first()._2._1, crs)
    feature(sc, res)
  }
  ...
  def union(implicit sc: SparkContext, featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
            featureRDD2: RDD[(String, (Geometry, Map[String, Any]))], crs: String = "EPSG:4326"): RDD[(String, (Geometry, Map[String, Any]))] = {
    val res = Geometry.union(featureRDD1.first()._2._1, featureRDD2.first()._2._1, crs)
    feature(sc, res)
  }
  ...
  def withDistance(featureRDD1: RDD[(String, (Geometry, Map[String, Any]))],
    ...
    val feature1 = reproject(featureRDD1, crs)
    val feature2 = reproject(featureRDD2, crs)
    feature1.first()._2._1.isWithinDistance(feature2.first()._2._1, distance).toString
  }
...
//      val pf: geotrellis.vector.Feature[vector.Point, Nothing] = PointFeature(p, data)
//      pf.asInstanceOf[PointFeature[Double]]
      PointFeature(p, data)
    }).collect()
    val t3 = System.currentTimeMillis()
